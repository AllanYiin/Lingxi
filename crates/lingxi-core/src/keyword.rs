    ...
    pub fn extract_keywords_with_options(
        ...
        let cand: Vec<Option<usize>> = tokens
            ...
            .map(|(token_index, t)| {
                ...
                let entity_candidate = options.proper_noun_enabled && proper_noun;
                if w.chars().count() < 2 || (!general_candidate && !entity_candidate) {
                    return None;
                }
        ...
        let proper_noun_weight = finite_unit(options.proper_noun_weight, 0.25);
        if options.proper_noun_enabled && proper_noun_weight > 0.0 {
            for i in 0..n {
        ...
        let max_ratio = finite_unit(options.proper_noun_max_ratio, 0.4);
        select_with_proper_noun_cap(
            &order,
            &metadata,
            top_k,
            options.proper_noun_enabled,
            max_ratio,
        )
        .into_iter()
        .map(|i| Keyword {
