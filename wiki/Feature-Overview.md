...
摘要必定先做結構感知子句抽取，再以 LingXi 分詞及 CKIP 詞性建立內容詞向量。候選會標記專有名詞、否定詞、括號／引號／Markdown 粗體、條列項目、函數／工具等物件名、日期、數字、帶單位數值與全大寫縮略語；`不只`／`不僅` 明確排除於否定訊號之外。括號或引號中的縮略語視為定義性內容並硬保留。句子相似度支援對稱 BM25（預設）與詞頻 cosine；TextRank 排名後以詞彙及有限規則型語意重疊門檻降低重複。

`top_k` 是一般候選的軟上限，不是固定輸出數量；條列、日期、數字與括號內縮略語等硬保留事實可超過此值。一般候選使用 0 到 1 的絕對 `explainability` 分數，預設門檻為 `0.35`；分數由相關性 45%、新增覆蓋 20%、新穎性 10% 與上述可辨識訊號覆蓋 25% 組成。它不是「相較前一句增加 15%」之類的固定邊際比例。若 Markdown 標題與條列已占主要內容，則判定為已壓縮的結構化筆記並完整保留。輸出會帶回各分項與訊號，讓納入理由可以檢查。
