...
進階 API 可另設共現窗口、阻尼、迭代／收斂條件、最短詞長與停用詞；既有 API 預設行為維持不變。

## 7. 中文斷句、子句與關鍵短語

句界 API 依中文終止標點與換行切句。子句 API 再依逗號、分號與冒號抽取，但不會切開括號、引號、反引號、Markdown `**粗體**`、數字千分位或尚待後果的條件前件；條列的每一行保有獨立句域。兩者都保留原文順序與 UTF-8 byte offset。關鍵短語則把原文中相鄰的高排名關鍵詞重組，回傳權重、出現次數與所有原文 spans。
