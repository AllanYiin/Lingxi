...
- 保留邊界：Markdown 條列、日期與數字屬於重要事實；帶單位數值優先級最高，不受一般可解釋性門檻或子句上限淘汰。
- 結構化筆記：當 Markdown 標題與條列已占主要內容時，判定為作者已壓縮的重點筆記，完整保留原文而不再次簡化。
- 語意與切句：`不只`／`不僅` 不標成否定；以逗號結尾的條件或數值前件必須與後果保留在同一子句。
- 縮略語：`FOMO`、`ETF`、`KOSPI` 等全大寫縮略語需有獨立診斷訊號；位於括號或引號中的縮略語視為定義性內容，硬保留且不受一般門檻或軟上限淘汰。
...
| Low-frequency | 調整一般候選軟上限與可解釋性門檻 |
...
8. 條列、日期與數字即使超過 `maxClauses` 或低於一般門檻仍被保留，診斷頁以不同色標顯示。
9. `不只`／`不僅` 的否定計數為 0；「每天久坐超過10小時，…後果…」不得被截成孤立前件。
10. `Fear Of Missing Out（FOMO）` 所在子句必須納入摘要，並同時顯示「強調」與「縮略語」訊號。
