...
- 摘要邊界：只能抽取原文 block／子句，不生成、不改寫、不對外傳送輸入。
- 結構邊界：先解析 paragraph、heading、code、list、blockquote、table、HTML 與 thematic break；code、table、HTML 及短清單項完整保留，長清單項只摘要其 prose。
- 預算邊界：`maxBlocks` 只限制可排名的 paragraph／blockquote；完整保留結構不計入預算，否定語意閉包可超過 block 內 clause 上限但必須在報告中說明。
- 重點訊號：專名、金額、日期、數量、裸數字、縮略語與物件名皆為軟加權，不得單憑訊號繞過排名門檻。
- 語意與切句：`不只`／`不僅`、`非常`、`是否`、`未來`、`否則` 不標成否定；英文 `not only`、`whether or not`、`otherwise` 亦不得誤判；條件／數值前件、定義句及「先…再…」處置鏈必須保留為完整語意單位。
- 縮略語：`FOMO`、`ETF`、`KOSPI` 等全大寫縮略語需有獨立診斷訊號；位於括號或引號中的縮略語提高 portable signal 分數，但不得單獨繞過 `maxBlocks` 或門檻。
...
- 輸入：textarea 中的 UTF-8 文字、最大 prose block 數、可解釋性門檻。
- 輸出：schema v2 結構感知摘要、tiktoken 估算、計算耗時、逐 block／子句診斷與預算超額理由。
...
7. 短 Markdown 條列、code、table 與 HTML block 的輸出與原文 slice 完全相同；長清單項保留 marker、巢狀結構與否定 clause。
8. `maxBlocks` 只限制 paragraph／blockquote；日期、金額與裸數字只提高權重，不得單憑訊號排擠明示的研究結論。
9. `不只`／`不僅`／`非常`／`是否`／`未來`／`否則` 及英文反例的否定計數為 0；有效否定不得從已入選容器中消失。
10. `Fear Of Missing Out（FOMO）…是指…` 必須以完整定義納入摘要，並同時顯示「強調」與「縮略語」訊號。
11. 「先…再…」處置鏈不得輸出只含後半段的懸空子句。
12. 品質回歸至少涵蓋 block offset、code/list 保留、定義完整性、無關日期、中英否定誤判、block 預算、跨端 golden parity 與語意懸空。
